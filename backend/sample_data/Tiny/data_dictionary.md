...
Generator `1.0.0`, contract version `1`, profile `tiny`, seed `20260923`.
...
|---|---|---|
| `missing_counterparty` | 3.00% | Loaded; lowers referential-integrity metric; drives the Entity 360 banner (produced: 108) |
| `missing_device` | 5.00% | Loaded; feature marked `DATA_UNAVAILABLE` (produced: 184) |
| `malformed_date` | 0.40% | Quarantined `INVALID_DATE_FORMAT` (produced: 8) |
| `invalid_currency` | 0.20% | Quarantined `INVALID_CURRENCY` (produced: 9) |
| `invalid_amount` | 0.20% | Quarantined `INVALID_AMOUNT` (produced: 12) |
| `unresolved_account` | 0.50% | Held to end of batch, then quarantined `UNRESOLVED_ACCOUNT` (produced: 25) |
| `exact_duplicate` | 1.00% | Suppressed by idempotency, still counted (produced: 39) |
| `idempotency_conflict` | 0.05% | Quarantined `IDEMPOTENCY_CONFLICT` (produced: 1) |
| `late_arrival` | 1.50% | Flagged `LATE_ARRIVAL`, triggers targeted re-evaluation (produced: 53) |
| `missing_bo` | 4.00% | Loaded; flagged `BO_MISSING`; feeds a risk factor (produced: 1) |
| `placeholder_address` | 2.00% | Loaded; produces a `LOW_SPECIFICITY` graph edge (produced: 2) |
| `truncated_name` | 3.00% | Loaded; some become `NOT_SCREENABLE`, others `HIGH_FREQUENCY_NAME` (produced: 4) |
